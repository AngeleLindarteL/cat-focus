...
import type { DeleteUsageModalProps } from "@/modules/usage/components/DeleteUsageModal/interfaces";

export function DeleteUsageModal({
  getTranslation,
  isOpen,
  onConfirm,
  onCancel,
      ...
      <div className="w-full max-w-sm rounded-[28px] border border-stone-200 bg-white p-6 shadow-xl">
        <div className="space-y-2">
          <h3 className="text-lg font-semibold text-stone-900">
            {getTranslation(TranslationKey.UsageDeleteConfirmTitle)}
          </h3>
          <p className="text-sm leading-6 text-stone-600">
            {getTranslation(TranslationKey.UsageDeleteConfirmDescription)}
          </p>
        </div>
        <div className="mt-5 flex justify-end gap-3">
          ...
          >
            {getTranslation(TranslationKey.UsageCancel)}
          </button>
          <button
            type="button"
            onClick={onConfirm}
          ...
          >
            {getTranslation(TranslationKey.UsageDeleteConfirmAction)}
          </button>
        </div>
      </div>
    </div>
